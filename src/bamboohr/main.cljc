(ns bamboohr.main
  "Kotodama WASM entrypoint for the Bamboohr clean-room actor (L4) — Clojure port.
  ...
  facts (`bamboohr.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "bamboohr")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"bamboohr.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "bamboohr-compat" :tier tier :entities entities} 200])
...
(defn start! [] :bamboohr-compat/ready)
